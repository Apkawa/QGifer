...
void TextWidget::init() {
    ...
    connect(lineEdit, SIGNAL(textChanged(
                                     const QString&)), this, SLOT(update()));
    connect(lineEdit, SIGNAL(textChanged(
                                     const QString&)), this, SLOT(validate()));
    connect(tcEdit, SIGNAL(textChanged(
                                   const QString&)), this, SLOT(update()));
    connect(ocEdit, SIGNAL(textChanged(
                                   const QString&)), this, SLOT(update()));
    connect(fontComboBox, SIGNAL(currentFontChanged(
                                         const QFont&)), this, SLOT(update()));
...
QImage TextWidget::renderText() const {
    ...
    return TextRenderer::renderText(lineEdit->text(),
                                    font,
                                    QColor(tcEdit->text()),
                                    QColor(ocEdit->text()),
                                    outlineBox->value());
